...
import com.alurahotel.jdbc.controller.HuespedController;
import com.alurahotel.jdbc.controller.ReservaController;
...
public class Busqueda extends JFrame {
	...
	public Busqueda() {
		...
		btnEditar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (scroll_tableHuespedes.isShowing()) {
					modificarHuesped();
					limpiarTablaHuesped();
					cargarTablaHuesped();
				} else {
...
// Acciones del boton de Eliminar
...
// Acciones del boton de Editar
	private void modificarHuesped() {
		...
		Optional.ofNullable(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), tbHuespedes.getSelectedColumn()))
				.ifPresentOrElse(fila -> {
					...
					var filasModificadas = this.huespedController.modificar(nombre, apellido, fechaNacimiento,
	...
	private void modificarReserva() {
		...
		Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
				.ifPresentOrElse(fila -> {
					Integer id = Integer.valueOf(modelo.getValueAt(tbReservas.getSelectedRow(), 0).toString());
					String fechaEntrada = String.valueOf(modelo.getValueAt(tbReservas.getSelectedRow(), 1));
					String fechaSalida = String.valueOf(modelo.getValueAt(tbReservas.getSelectedRow(), 2));
					int valor = (int) modelo.getValueAt(tbReservas.getSelectedRow(), 3);
					String formaPago = String.valueOf(modelo.getValueAt(tbReservas.getSelectedRow(), 4));
					var filasModificadas = this.reservaController.modificar(fechaEntrada, fechaSalida, valor, formaPago,
							id);
...
// Buscar
	private void buscarHuesped(String texto) {
		if (!verificarEsNumero(texto)) {
			var huespedes = this.huespedController.filtroApellido(texto);
			huespedes.forEach(huesped -> modeloHuesped.addRow(new Object[] { huesped.getId(), huesped.getNombre(),
					huesped.getApellido(), huesped.getFechaDeNacimiento(), huesped.getNacionalidad(),
					huesped.getTelefono(), huesped.getIdReserva() }));
		} else {
			var huespedes = this.huespedController.filtroId(Integer.valueOf(texto));
			huespedes.forEach(huesped -> modeloHuesped.addRow(new Object[] { huesped.getId(), huesped.getNombre(),
					huesped.getApellido(), huesped.getFechaDeNacimiento(), huesped.getNacionalidad(),
					huesped.getTelefono(), huesped.getIdReserva() }));
		}
	...
	private void buscarReserva(String texto) {
		if (!verificarEsNumero(texto)) {
			var reservas = this.reservaController.filtroApellido(texto);
			reservas.forEach(reserva -> modelo.addRow(new Object[] { reserva.getId(), reserva.getFechaEntrada(),
					reserva.getFechaSalida(), reserva.getValor(), reserva.getFormaPago() }));
		} else {
			var reservas = this.reservaController.filtroId(Integer.valueOf(texto));
...
// Verificar si el dato escrito es un numero o un texto
	private boolean verificarEsNumero(String texto) {
